  ...
  describe('saveComponent', () => {
    ...
    test('saves components with/without model.js', () => {
      ...
      return fn(store, { uri, data, prevData }).then(() => {
        expect(store.commit).toHaveBeenCalledWith('UPDATE_COMPONENT', { uri, data });
  ...
  describe('removeComponent', () => {
    ...
    test('accepts an element as the second argument', () => {
      ...
      return fn(store, data).then(() => {
        expect(store.commit).toHaveBeenCalledWith('REMOVE_COMPONENT', { uri: 'domain.com/_components/bar' });
      });
    });
