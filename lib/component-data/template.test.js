...
describe('component template', () => {
  ...
  beforeEach(() => {
    ...
    components.getTemplate.returns((data) => `hi ${data.locals.edit} ${data.text} (${data._ref}) (${data.componentVariation})!`);
  ...
  describe('render', () => {
    ...
    it('renders with data and locals', () => {
      fn('foo', { _ref: 'foo', text: 'friend', componentVariation: 'foo_bar' }).then((res) => {
        expect(res).to.equal('hi true friend (foo) (foo_bar)!');
      });
    });
    ...
    it('adds componentVariation if not passed in', () => {
      fn('foo', { _ref: 'foo', text: 'friend' }).then((res) => {
        expect(res).to.equal('hi true friend (foo) (foo)!');
      });
    });
  });
});
