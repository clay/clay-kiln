...
import { UPDATE_COMPONENT, ADD_SCHEMA } from './mutationTypes';
import { getData, getSchema, getModel } from '../core-data/components';
...
function addChildrenToParent(children, uri, data) {
  ...
  _.forOwn(children, (val, key) => {
    promises[key] = getDefaultData(val).then((defaultData) => {
...
function createComponentAndChildren({name, data}) {
  ...
  return props({
    ...
    defaultData: getDefaultData(name)
  }).then(({defaultData}) => {
