...
function findIndex(data, uri) {
  const index = _.findIndex(data, (item) => item[refProp] === uri);
 ...
 * @param {array} data
 * @param {array} uriList
 * @param {number} [overrideIndex] to use in place of currentURI if we've already removed it (when replacing a list)
 * @return {Promise}
 */
function addComponentToComponentList(store, { currentURI, parentURI, path, data, uriList, overrideIndex }) {
  const index = overrideIndex || findIndex(data, currentURI),
    uriObjects = _.map(uriList, (listUri) => {
      return { [refProp]: listUri };
    }),
    newList = _.slice(data, 0, index + 1).concat(uriObjects).concat(_.slice(data, index + 1));

  return store.dispatch('saveComponent', { uri: parentURI, data: { [path]: newList } });
}
 ...
 * @param {string} parentURI
 * @param {string} path
 * @param {string|array} uri of the component(s) to add
 * @param {array|object|string} [overrideData] to use in place of the store data if we're replacing a list/prop
 * @param {number} [overrideIndex] to use in place of currentURI if we've already removed it (when replacing a list)
 * @return {Promise} with the new component's el
 */
export function addComponent(store, { currentURI, parentURI, path, uri, overrideData, overrideIndex }) {
  const data = overrideData || getData(parentURI, path),
    uriList = _.isString(uri) ? [uri] : uri; // if we're adding a single component to a list, wrap it in an array

  let promise;

  if (_.isArray(data)) {
    // list in a component
    promise = addComponentToComponentList(store, { currentURI, parentURI, path, data, uriList, overrideIndex });
  } else if (_.isObject(data)) {
    // prop in component
    if (!_.isString(uri)) {
      // you can't add multiple uris to a component prop!
      throw new Error(`Cannot add multiple children to a component prop! ${uri.join(', ')}`);
    }
    ...
    const index = _.findIndex(data, currentURI) || data.length - 1,
      newList = _.slice(data, 0, index + 1).concat(uriList).concat(_.slice(data, index + 1));
  ...
  return promise.then(() => {
    ...
    return find(`[${refAttr}="${_.last(uriList)}"]`);
  });
}

/**
 * remove the current uri and add one or more uris in its place
 * @param  {object} store
 ...
 * @return {Promise}
 */
export function replaceComponent(store, { currentURI, parentURI, path, uri }) {
  const list = getData(parentURI, path);

  if (_.isArray(list)) {
    // list in a component
    let currentData = _.find(list, (item) => item[refProp] === currentURI),
      overrideIndex = list.indexOf(currentData),
      overrideData = _.without(list, currentData);

    return store.dispatch('addComponent', { parentURI, path, uri, overrideData, overrideIndex });
  } else if (_.isObject(list)) {
    throw new Error('Todo: support component replacement in component properties!');
  } else if (_.isString(list)) {
    throw new Error('Todo: support component replacement in page data!');
  }
}
...
function getDefaultData(name, store) {
  ...
  if (data) {
    ...
    return getObject(defaultURI).then((res) => {
      store.commit(UPDATE_COMPONENT, { uri: defaultURI, data: res });
      return res;
    });
  }
}
  ...
  _.forOwn(children, (val, key) => {
    promises[key] = getDefaultData(val, store).then((childDefaultData) => {
      ...
      return props({
        ...
        store.commit(UPDATE_COMPONENT, { uri: res.data[refProp], data: res.data });
