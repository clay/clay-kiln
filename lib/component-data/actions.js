...
import { UPDATE_COMPONENT, REVERT_COMPONENT, RENDER_COMPONENT } from './mutationTypes';
 ...
 * @returns {Promise}
 */
function addComponentsToPageArea(store, {path, replace, components}) {
...
export function addComponents(store, { currentURI, parentURI, path, replace, components }) {
  ...
  return store.dispatch('unfocus').then(() => {
    if (_.isArray(data)) {
      ...
      return addComponentsToPageArea(store, {path, replace, components});
    };
  });
}
