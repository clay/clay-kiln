 ...
 * @param {boolean} [validate=true] set to false to prevent page revalidation
 * @return {Promise}
 */
export function saveComponent(store, {uri, data, eventID, snapshot, prevData, validate}) {
...
/**
 * Revalidate the page if the the health or publish drawer is open
 * @param {object} store 
 */
function revalidate(store) {
  // we only need to actually run validation if the publish or health drawer is open
  if (_.get(store, 'state.ui.currentDrawer') === 'publish' ||
    _.get(store, 'state.ui.currentDrawer') === 'health'
  ) {
...
export function removeComponent(store, el) {
  ...
  if (_.isArray(list)) {
    ...
    promise = saveComponent(store, { uri: parentURI, data: { [path]: newList }, validate: false});
  } else if (_.isObject(list)) {
    promise = saveComponent(store, { uri: parentURI, data: { [path]: {} }, validate: false});
  ...
  return promise.then(() => {
    ...
    return revalidate(store);
  })
  .then(() => find(`[${refAttr}="${prevURI}"]`))
}
