...
function clientSave(uri, data, store) {
  return modelSave(uri, data)
    ...
    .then((html) => {
      ...
      const el = decorateURI(uri, html); // decorate the new html
      // todo: replace multiple els if this component instance has multiple elements on the page
      // todo: decorate other els
      // todo: not sure if we can decorate in vdom or whatnot, not sure if we can do reactive re-rendering in a more declarative way
      console.log('decorated el:', el)
    })
