...
function clientSave(uri, data, oldData, store, {eventID, snapshot, paths}) { // eslint-disable-line
  return modelSave(uri, data)
    ...
    .then((savedData) => {
      // queue the API save; propagate errors so the outer .catch handles
      // logging + revert. we don't use the api response data (hooks=false
      // prevents models from re-running on the server).
      return addToQueue(save, [uri, savedData, false], 'save')
        .catch((e) => {
          store.commit(REVERT_COMPONENT, { uri, oldData });

          return Promise.reject(e); // re-throw so outer .catch logs + reverts
        })
        .then(() => savedData);
    })
