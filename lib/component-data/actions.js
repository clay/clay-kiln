...
import { UPDATE_COMPONENT, REVERT_COMPONENT, REMOVE_COMPONENT, CURRENTLY_SAVING, OPEN_ADD_COMPONENT, CLOSE_ADD_COMPONENT, UPDATE_SCHEMA_PROP, CURRENTLY_RESTORING } from './mutationTypes';
...
function addComponentsToPageArea(store, {currentURI, path, replace, components}) {
  ...
  return create(components)
    .then((newComponents) => {
      store.dispatch('addCreatedComponentsToPageArea', {newComponents, store, currentURI, path, replace, index, data});
    });
}
