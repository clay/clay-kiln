 ...
 * @return {Promise}
 */
function clientSave(uri, data, oldData, store, {eventID, snapshot}) { // eslint-disable-line
  return modelSave(uri, data, {eventID, snapshot})
    .then((savedData) => {
      ...
      addToQueue(save, [uri, data])
        ...
        .catch((e) => {
          ...
          store.commit(RENDER_COMPONENT, { uri, data: oldData, snapshot });
    ...
    .then((renderableData) => {
      ...
      store.commit(RENDER_COMPONENT, { uri, data: renderableData, snapshot });
 ...
 * @return {Promise}
 */
function serverSave(uri, data, oldData, store, {snapshot}) { // eslint-disable-line
  return addToQueue(save, [uri, data])
    .then((res) => {
      store.commit(UPDATE_COMPONENT, {uri, data: res});
      store.commit(RENDER_COMPONENT, { uri, data: res, snapshot });
      return res;
    })
    .catch((e) => {
      store.commit(REVERT_COMPONENT, {uri, oldData});
 ...
 * @return {Promise}
 */
function serverSaveAndRerender(uri, data, oldData, store, {snapshot}) { // eslint-disable-line
  ...
  return addToQueue(saveForHTML, [uri, data])
    .then((html) => { // PUT to html before getting new data
      return addToQueue(getObject, [uri])
        .then((res) => {
          store.commit(UPDATE_COMPONENT, {uri, data: res});
          store.commit(RENDER_COMPONENT, { uri, html, snapshot });
          return res;
        })
        // otherwise revert the data
        .catch((e) => {
          store.commit(REVERT_COMPONENT, {uri, oldData});
          store.commit(RENDER_COMPONENT, { uri, html: oldHTML, snapshot });
          logSaveError(uri, e, store);
        });
    });
}
 ...
 * @return {Promise}
 */
export function saveComponent(store, {uri, data, eventID, snapshot}) {
  ...
  if (model && template) {
    ...
    promise = clientSave(uri, dataToSave, oldData, store, {eventID, snapshot});
  } else if (template) {
    ...
    promise = serverSave(uri, dataToSave, oldData, store, {snapshot});
    ...
    promise = serverSaveAndRerender(uri, dataToSave, oldData, store, {snapshot});
