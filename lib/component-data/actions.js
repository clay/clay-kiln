 ...
 * @param  {object} store
 * @param  {Element} data  el || {el, msg}
 * @return {Promise}
 */
export function removeComponent(store, data) {
  const componentEl = typeof data === 'object' ? getComponentEl(data.el) : getComponentEl(data),
  ...
  return promise.then(() => {
    store.commit(REMOVE_COMPONENT, {uri, msg: msg});
...
export function openAddComponent(store, { currentURI, parentURI, path, pos }) {
  const schemaURI = isPage(parentURI) ? _.get(store, 'state.page.data.layout') : parentURI,
