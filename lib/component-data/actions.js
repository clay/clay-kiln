  ...
  return modelSave(uri, data)
    .then((savedData) => {
      ...
      addToQueue(save, [uri, data])
        .then(() => {
          // we don't care about the data we got back from the api
          store.commit(SAVE_SUCCESS, uri);
        }).catch((e) => {
          store.commit(SAVE_FAILURE, {uri, oldData});
          logSaveError(uri, e);
          return renderTemplate(uri, oldData)
            .then((html) => decorateURI(uri, html)); // revert the html (of all matching elements)
        });
...
function serverSave(uri, data, oldData, store) {
  store.commit(SERVER_SAVE_PENDING, {uri});

  return addToQueue(save, [uri, data])
    .then((res) => {
      store.commit(SERVER_SAVE_SUCCESS, {uri, data: res});
      return res;
    })
    .then((renderableData) => renderTemplate(uri, renderableData))
    .then((html) => decorateURI(uri, html))
    .catch((e) => {
      store.commit(SAVE_FAILURE, {uri, oldData});
      logSaveError(uri, e);
    });
...
function serverSaveAndRerender(uri, data, oldData, store) {
  ...
  return addToQueue(saveForHTML, [uri, data])
    .then((html) => decorateURI(uri, html))
    .then(addToQueue(getObject, [uri])) // get new data from api AFTER we re-render (saves us some time in the UI)
    .then((res) => {
      store.commit(SERVER_SAVE_SUCCESS, {uri, data: res});
      return res;
    })
