...
export function setFixedPoint(store) {
  ...
  if (!atEnd) {
    ...
    console.log('fixed point! latest:', _.last(history))
 ...
 * @param {object} store
 */
function saveChangedComponents(changedComponents, actionName, store) {
  _.forOwn(changedComponents, (data, uri) => store.dispatch('saveComponent', {uri, data, snapshot: false})); // don't pass event ID, do tell it to not create snapshots
}
...
export function undo(store) {
  ...
  if (!store.state.undo.atStart) {
    ...
    console.log('undo', store.state.undo.cursor, history)
    ...
    console.log('changed', changedComponents)
    saveChangedComponents(changedComponents, 'undo', store);
  }
}
...
export function redo(store) {
  ...
  if (!store.state.undo.atEnd) {
    ...
    console.log('redo', store.state.undo.cursor, history)
    ...
    console.log('changed', changedComponents)
    saveChangedComponents(changedComponents, 'redo', store);
  }
}
