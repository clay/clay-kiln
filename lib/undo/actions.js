...
function saveChangedComponents(changedComponents, store) {
  _.forOwn(changedComponents, ({data, prevData}, uri) => store.dispatch('saveComponent', {uri, data, snapshot: false, prevData})); // don't pass event ID, do tell it to not create snapshots
}
