...
import { CREATE_SNAPSHOT, REMOVE_SNAPSHOTS, UNDO, REDO } from './mutationTypes';
...
define('undo mutations', () => {
  ...
  describe(REMOVE_SNAPSHOTS, () => {
    it('removes snapshots after the passed-in cursor', () => {
      expect(fn({ undo: { history: ['a', 'b'] } }, 0)).to.eql({ undo: { history: ['a']}});
    });
  });
