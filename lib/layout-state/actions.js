...
import { postJSON, save, create, remove, getMeta, saveMeta } from '../core-data/api';
import { UPDATE_LAYOUT_STATE, SCHEDULE_LAYOUT, UNSCHEDULE_LAYOUT } from './mutationTypes';
import { searchRoute, scheduleRoute, refProp } from '../utils/references';
...
export function fetchLayoutState(store, preloadOptions = {}) {
  const sitePrefix = preloadOptions.prefix || _.get(store, 'state.site.prefix'),
    layoutUri = replaceVersion(preloadOptions.uri || _.get(store, 'state.layout.uri'));
