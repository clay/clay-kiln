 ...
 * @return {Promise}
 */
export function fetchLayoutState(store, { uri, prefix }) {
  const sitePrefix = prefix || _.get(store, 'state.site.prefix'),
    layoutUri = uri || _.get(store, 'state.page.data.layout');
 ...
 * @return {Promise}
 */
export function updateLayout(store, data) {
  const prefix = _.get(store, 'state.site.prefix'),
    uri = _.get(store, 'state.page.data.layout');
...
export function publishLayout(store) {
  ...
  return addToQueue(save, [`${replaceVersion(layoutUri, 'published')}`], 'published') // PUT @published with empty data, and it'll clone the latest data
    .then(() => store.dispatch('fetchLayoutState'));
}
