...
const saveLayoutMeta = _.debounce((uri, data, store) => saveMeta(uri, data).then((response) => {
...
export function updateLayout(store, data, preloadOptions = {}) {
  ...
  const currentState = _.get(store, 'state.layout.state', {}),
...
export function publishLayout(store) {
  ...
  return promise.then(() => save(replaceVersion(layoutUri, 'published')) // PUT @published with empty data, and it'll clone the latest data
