...
export function updatePageList(store, data = {}) {
  ...
  let title = _.get(store, 'state.page.listData.title'),
    ...
    publishTime = _.get(store, 'state.page.listData.publishTime'), // latest published timestamp, not first published
    updateTime = new Date(),
  ...
  store.commit(UPDATE_PAGE_LIST_DATA, { title, authors, updateTime, publishTime, users });
