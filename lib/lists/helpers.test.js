...
describe('list helpers', () => {
  ...
  it('add an item to a list', () => {
    const items = [
      ...
      tester = 'theta',
      updatedItems = helpers.addItem(items, tester);
  ...
  it('throw an error if trying to add an already existing item to a list', () => {
    ...
    expect(updatedItems).to.eql(false);
    expect(loggerStub.error).to.have.been.calledWith('This item already exists in this list!', {action: 'modifyList'});
  });

  it('checks whether or not the items in a list has property', () => {
    const items = [
        { text : 'alpha' },
        { text : 'beta' },
        { text : 'gamma'}],
    ...
    expect(helpers.getProp(items, tester)).to.eql(tester);
  });
