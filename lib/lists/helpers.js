 ...
 * @returns {array}
 */
export function addItem(items, testItem, testProperty) {
  const index = getItemIndex(items, testItem, testProperty);

  if (index !== -1) {
    log.error('This item already exists in this list!', {action: 'modifyList'});
    return false;
  } else {
    items.push(testItem);
    return items;
...
export function getProp(items, testProperty) {
  ...
  return stringProp && _.isString(stringProp) ? testProperty : null;
}
