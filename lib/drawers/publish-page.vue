<template>
  <div class="publish-drawer">
    ...
    <div class="publish-status">
      ...
      <ui-button v-if="isScheduled" class="status-undo-button" buttonType="button" color="red"
        @click.stop="unschedulePage">Unschedule</ui-button>
      <ui-button v-else-if="isPublished" class="status-undo-button" buttonType="button" color="red"
        @click.stop="unpublishPage">Unpublish</ui-button>
      <ui-button v-else-if="isArchived" class="status-undo-button" buttonType="button" color="red"
        @click.stop="archivePage(false)">Unarchive</ui-button>
    </div>

    <!-- publish actions -->
    <div class="publish-actions">
      <span class="action-message">{{ actionMessage }} <ui-icon-button v-if="showSchedule" icon="close"
          buttonType="button" type="secondary" color="default" size="small" tooltip="Clear Date/Time"
          tooltipPosition="left middle" @click.stop="clearScheduleForm"></ui-icon-button></span>
      <form class="schedule-form" @submit.prevent="schedulePage">
        <ui-datepicker class="schedule-date" color="accent" v-model="dateValue" :minDate="today"
          :customFormatter="formatDate" label="Date" :disabled="hasErrors"></ui-datepicker>
        <timepicker ref="timepicker" class="schedule-time" :value="timeValue" label="Time" :disabled="hasErrors"
          @update="updateTime"></timepicker>
      </form>
      <span class="action-info-message">Time Zone: {{ timezone }}</span>
      <ui-button v-if="showSchedule" :disabled="disableSchedule || isArchived || hasErrors || !isLayoutPublished"
        class="action-button" buttonType="button" color="orange" @click.stop="schedulePage">{{ actionMessage
        }}</ui-button>
      <ui-button v-else :disabled="checksPassed" class="action-button" buttonType="button" color="accent"
        @click.stop="publishPage">{{ actionMessage }}</ui-button>
      <span v-if="!isLayoutPublished && isAdmin" class="action-error-message" @click="goToLayout">Layout must be
        published first</span>
      <span v-else-if="!isLayoutPublished" class="action-error-message">Layout must be published first (by an
        admin)</span>
      <span v-else-if="hasErrors" class="action-error-message" @click="goToHealth">Please fix errors before
        publishing</span>
      <span v-else-if="hasWarnings" class="action-warning-message" @click="goToHealth">Please review warnings before
        publishing</span>
    </div>

    <!-- custom location -->
    <ui-collapsible :open="hasCustomLocation" class="publish-section publish-location" title="Custom URL"
      ref="uiCollapsiblePublish">
      <form class="publish-location-form" @submit.prevent="saveLocation">
        <span class="location-description">Designate a custom URL for this page. This should only be used for special
          cases, such as index pages and static pages.</span>
        <ui-textbox class="location-input" v-model="location" placeholder="/special-page.html"
          label="Enter Custom Location" :error="error" :invalid="isInvalid" @input="onLocationInput"></ui-textbox>
    ...
    <ui-collapsible class="publish-section publish-title" title="Page Title">
      <form class="publish-title-form" @submit.prevent="saveTitle">
        <span class="title-description">Manually update the page title that appears in the Clay Menu. Will be
          overwritten when saving components that set the title.</span>
        <ui-textbox class="title-input" v-model="title" label="Page Title"></ui-textbox>
        <ui-button class="title-submit" buttonType="submit" type="primary" color="default"
          :disabled="!title">Save</ui-button>
      </form>
    </ui-collapsible>

    <!-- archiving -->
    <ui-collapsible class="publish-section publish-archive" title="Archive Page">
      <span class="archive-help">You may archive any page that isn't published (or scheduled to be published). Archived
        pages will not show up in the Clay Menu unless you explicitly filter for them.</span>
      <ui-button class="archive-submit" buttonType="button" type="primary" color="red"
        :disabled="isScheduled || isPublished || isArchived" @click.stop="archivePage(true)">Archive</ui-button>
    </ui-collapsible>


    <!-- restore from published -->
    <ui-collapsible v-if="isPublished && hasChanges" class="publish-section publish-archive"
      title="Restore Published Version">
      <span class="archive-help">You may override all changes by restoring to the most recently published version of
        this page. <strong>Warning:</strong> this action cannot be undone, you will lose all changes applied to this
        post since the last publish.</span>
      <ui-button class="action-button" buttonType="button" type="primary" color="red"
        @click.stop="restorePageClick()">Restore</ui-button>
    </ui-collapsible>
  </div>
</template>
