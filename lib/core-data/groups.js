...
// TODO: define at some point, what values are underscored and which are not.
// By convention, underscored properties shouldn't be accessed by the developer, but
// that does not seem to be the case here.

/**
 * A class which contains a set of fields and a schema
...
/**
 * A set of directives for
 * TODO: verify this info
 * TODO: flesh this out
 * @typedef {object} Schema
 * @property {object} [Schema] - contains other schemas
 * @property {string} _description - user-facing description
 * @property {string} _label - user-facing label
 * @property {object} _groups - ???
 * @property {Has} _has all the other stuff it "_has"
 * @property {object} _component - TODO: ????
 */

/**
 * TODO: don't know how to define this.
 * @typedef {object} Has
 * @property {string} input - the type of input this is
 * @property {array.string} options - options for radio/dropdown type input
 * @property {array.Schema} props - child schemas? child values? TODO
 * @property {string} help - help text for
 */

/**
 * An interactive placeholder to indicate to users that this is editable
 * @typedef {object} Placeholder
 * @property {string} height - css property (height) of container
 * @property {string} ifEmpty - directive for kiln to fill unpopulated data with.
 * see ... TODO
 * @property {array} props - TODO
 */
...
function newFieldSet(data, schema, path = '', componentName = '') {
  const { prop, index, subProp } = analyzePath(path);
  ...
  if (singleProp && _.get(data, path) !== undefined) {
    return {
      fields: { [subProp || prop]: _.get(data, path) },
      schema: subProp ? findSubSchema() : schema[prop]
    };
  };
  ...
  // find the subschema on the schema if subprop is speficied
  // TODO: this is SO UNCLEAR
  function findSubSchema() {
    return _.get(schema, `${prop}._has.props`, [])
      .find(({ prop }) => prop === subProp);
  }

  // break path into constituent path parts
  function analyzePath(path) {
    const prop = path.split('[')[0],
      indexStr = (/\[.[0 - 9]?\]/).exec(path),
      index = indexStr && parseInt(indexStr[0].replace(/\[|\]/g, ''), 10),
      subProp = path.split('].')[1];
  ...
  function overrideVariations({ fields, sections }) {
    if (path === 'settings' &&
      ...
      !_.has(fields, 'componentVariation')) {
...
export function get(uri, path) {
  ...
  // return a new FieldSet by grabbing the data/schema from the store
