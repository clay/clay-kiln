...
describe('behaviors2input', () => {
  ...
  describe('hasAnyBehaviors', () => {
    ...
    it('returns true if schema has any old-style behaviors', () => {
      expect(fn({ a: { _has: { fn: 'text',  type: 'url' } } })).to.equal(true);
    });

    it('returns false if schema has no old-style behaviors', () => {
      expect(fn({ a: { _has: { input: 'text',  type: 'url' } } })).to.equal(false);
    });
  });
  ...
  describe('convertSchema', () => {
    ...
    it('ignores _version, _description, _devDescription, and _confirmRemoval', () => {
      const metadata = {
        _version: 1,
        _description: 'hi',
    ...
    it('converts (text) type=time to actual time input', () => {
      ...
      }, name, path)).to.eql({ _has: 'timepicker' });
    });
    ...
    it('adds autocomplete config to input', () => {
      expect(fn({
        [path]: {
          _has: [{
            fn: 'simple-list',
