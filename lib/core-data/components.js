...
import { hasBehaviors, convertSchema } from './behaviors2input';
...
export function getSchema(uri, path, testStore) {
  ...
  if (_.some(Object.keys(schema), (prop) => hasBehaviors(prop, schema))) {
    console.warn(`Behaviors API is deprecated and will be removed in the next major Kiln version: Please update ${getComponentName(uri)}${path ? ' → ' + path : ''} to the Inputs API`);
