...
import { displayProp, fieldProp, behaviorKey, groupsProp, componentListProp, componentProp, descProp } from '../utils/references';
...
function convertSchemaProp(schema, prop) {
  if (_.includes([descProp, versionProp])) {
...
function convert(schema, name, path) {
  ...
  return path ? convertSchemaProp(schema, path) : _.mapValues(schema, (val, key, fullObj) => convertSchemaProp(fullObj, key));
}
