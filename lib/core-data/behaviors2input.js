...
function mungeMainInput(mainInput, behaviors, prop) { // eslint-disable-line
  ...
  if (mainInput[behaviorKey] === 'textarea') {
    ...
    console.error(`'${prop}' (datetime-local) must be converted to two separate date and time fields! Editing this field in kiln will set the time to 12:00 AM on the date specified.`);
    input[inputProp] = 'datepicker';
  } else {
...
function convertMainInput(schema, prop) {
  ...
  if (inlineWysiwyg) {
    ...
    console.warn(`'${prop}' has no behaviors specified!`);
  }
...
function convertSchemaProp(schema, prop) {
  if (_.includes([descProp, versionProp], prop)) {
    // pass through _version and _description
    return schema[prop];
    ...
    // convert group
    return schema[prop]; // todo: convert groups
  } else {
    // convert field
...
function convertImplicitSettings(newSchema, schema) {
  // todo: convert implicit settings into explicit settings group
}
...
function convert(schema, name, path) {
  ...
  console.warn(`${name}${path ? ' → ' + path : ''} must be updated to the Inputs API, as the Behaviors API is deprecated (and will be removed in the next major Kiln version)`);
