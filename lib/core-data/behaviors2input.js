...
export function hasAnyBehaviors(schema) {
  return _.some(Object.keys(schema), (prop) => hasBehaviors(prop, schema));
}
...
function mungeMainInput(mainInput, behaviors, prop) { // eslint-disable-line
  ...
  if (mainInput[behaviorKey] === 'textarea') {
    ...
    log.error(`'${prop}' (datetime-local) must be converted to two separate date and time fields! Editing this field in kiln will set the time to 12:00 AM on the date specified.`, { action: 'mungeMainInput' });
    input[inputProp] = 'datepicker';
  } else {
...
function convertMainInput(schema, prop) {
  ...
  if (inlineWysiwyg) {
    ...
    log.warn(`'${prop}' has no behaviors specified!`, { action: 'convertMainInput' });
  }
...
function convertSchemaProp(schema, prop) {
  if (_.includes([descProp, devDescProp, versionProp], prop)) {
...
function convert(schema, name, path) {
  ...
  log.warn(`${name}${path ? ' → ' + path : ''} must be updated to the Inputs API, as the Behaviors API is deprecated (and will be removed in the next major Kiln version)`, { action: 'convert' });
