...
function deepReduce(result, tree, fn) {
  if (_.isObject(tree) && tree._ref && isComponent(tree._ref)) {
...
function reduceTree(original, layoutSchema) {
  const pageAreas = _.filter(Object.keys(layoutSchema), function (key) {
      return key !== '_description' && _.get(layoutSchema, `${key}._componentList.page`) || _.get(layoutSchema, `${key}._component.page`);
    }),
    layoutAreas = _.filter(Object.keys(layoutSchema), function (key) {
      return key !== '_description' && !_.includes(pageAreas, key);
    }),
