...
describe('api', () => {
  ...
  describe('save', () => {
    ...
    test('disables server-side hooks', () => {
      ...
      return fn(uri, goodObj, false).then(() => {
        expect(rest.send).toHaveBeenCalledWith('http://domain.com/thing?hooks=false', {body: '{"ok":"good"}', credentials: 'same-origin', headers: {'Content-Type': 'application/json; charset=UTF-8', 'X-Clay-User': undefined}, method: 'PUT', url: 'domain.com/thing?hooks=false'});
      });
    });
  });
  ...
  describe('create', () => {
    ...
    test('disables server-side hooks', () => {
      respond(goodObj);
      return fn(uri, goodObj, false).then(() => {
        expect(rest.send).toHaveBeenCalledWith('http://domain.com/thing?hooks=false', {body: '{"ok":"good"}', credentials: 'same-origin', headers: {'Content-Type': 'application/json; charset=UTF-8', 'X-Clay-User': undefined}, method: 'POST', url: 'domain.com/thing?hooks=false'});
      });
    });
  });
