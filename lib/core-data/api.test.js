...
describe('api', () => {
  ...
  describe('getSchema', () => {
    ...
    test('gets schema for a component', () => {
      ...
      return fn('domain.com/_components/foo').then(() => {
        expect(rest.send).toHaveBeenCalledWith('http://domain.com/_components/foo/schema', {credentials: 'same-origin', headers: {'X-Clay-User': undefined}, method: 'GET', url: 'domain.com/_components/foo/schema'});
      });
    });
  });
  ...
  describe('getHTML', () => {
    ...
    test('gets html with query (deprecated)', () => {
      ...
      return fn(uri, '&hi').then(() => {
        expect(rest.send).toHaveBeenCalledWith('http://domain.com/thing.html?edit=true&sitecss=false&js=false&hi', {credentials: 'same-origin', headers: {'X-Clay-User': undefined}, method: 'GET', url: 'domain.com/thing.html?edit=true&sitecss=false&js=false&hi'});
      });
    });
  });
  ...
  describe('save', () => {
    ...
    test('disables server-side hooks', () => {
      ...
      return fn(uri, goodObj, false).then(() => {
        expect(rest.send).toHaveBeenCalledWith('http://domain.com/thing?componenthooks=false', {body: '{"ok":"good"}', credentials: 'same-origin', headers: {'Content-Type': 'application/json; charset=UTF-8', 'X-Clay-User': undefined}, method: 'PUT', url: 'domain.com/thing?componenthooks=false'});
      });
    });
  });
  ...
  describe('create', () => {
    ...
    test('disables server-side hooks', () => {
      respond(goodObj);
      return fn(uri, goodObj, false).then(() => {
        expect(rest.send).toHaveBeenCalledWith('http://domain.com/thing?componenthooks=false', {body: '{"ok":"good"}', credentials: 'same-origin', headers: {'Content-Type': 'application/json; charset=UTF-8', 'X-Clay-User': undefined}, method: 'POST', url: 'domain.com/thing?componenthooks=false'});
      });
    });
  });
