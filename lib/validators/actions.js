  ...
  Object.keys(schemas).forEach((schemaName) => {
    ...
    if (schemas[schemaName].validation ) {
      ...
      schemas[schemaName].validation.forEach((validationRule) => {
        // We're only interested in validationRules with a type that matches the errorlevel (error, warning, etc.)
        if (validators[validationRule.validator].type === errorLevel) {
