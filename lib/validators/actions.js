 ...
 * @returns {boolean}
 */
function isGlobalMetadataError({ scope, type, uri }) {
 ...
 * @returns {boolean}
 */
function isGlobalMetadataWarning({ scope, type, uri }) {
 ...
 * @returns {boolean}
 */
function isSpecificMetadataWarning({ scope, type, uri }, pageUri) {
 ...
 * @returns {boolean}
 */
function isSpecificMetadataError({ scope, type, uri }, pageUri) {
...
export function validate(store) {
  const state = store.state,
    ...
    warningValidators = _.filter(validators, (v) => v.type === 'warning' && !v.scope),
    errorValidators = _.filter(validators, (v) => v.type === 'error' && !v.scope),
  ...
  return props({
    ...
    metadataErrors: runMetaValidators(uri, [
      ...globalMetadataErrorValidators,
      ...specificMetadataErrorValidators
    ]),
    metadataWarnings: runMetaValidators(uri, [
    ...
    const errors = _.filter(_.flatten(results.errors), hasItems),
      ...
      metadataErrors = _.filter(_.flatten(results.metadataErrors), hasItems),
      metadataWarnings = _.filter(_.flatten(results.metadataWarnings), hasItems);
