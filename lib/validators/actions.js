...
export function runKilnjsValidators(schemas = {}, components = {}) {
  ...
  Object.keys(components).forEach((uri) => {
    ...
    if (_.get(schemas, `${componentName}.validation`)) {
      _.get(schemas, `${componentName}.validation`, []).forEach((validationRule) => {
...
export function getSchemasWithValidationRules(schemas = {}) {
  ...
  Object.keys(schemas).forEach(schemaName => _.get(schemas, `${schemaName}.validation`, []) ? validationSchemas[schemaName] = _.cloneDeep(schemas[schemaName]) : null);
