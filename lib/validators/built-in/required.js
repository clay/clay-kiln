...
import { hasValidation, getListProps } from '../helpers';
...
function hasEmptyRequiredListItems(val, fieldSchema) {
  ...
  return !!_.find(props, (propConfig) => hasValidation('required', propConfig) && !!_.find(val, (item) => isEmpty(item[propConfig.prop])));
}
  ...
  return _.reduce(state.components, (result, component, uri) => {
    if (!_.isEmpty(component)) {
      ...
      _.forOwn(schema, (fieldSchema, fieldName) => {
        if (hasValidation('required', fieldSchema)) {
