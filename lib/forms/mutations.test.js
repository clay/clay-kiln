...
describe('form mutations', () => {
  test(`sets current form for ${OPEN_FORM}`, () => {
    expect(lib[OPEN_FORM]({}, { uri, path, inline, fields, schema, initialOffset, appendText, pos })).toEqual({ ui: { currentForm: { uri, path, inline, fields, schema, initialOffset, appendText, pos } } });
  });

  test(`sets current form for ${CLOSE_FORM}`, () => {
    expect(lib[CLOSE_FORM]({ ui: { currentForm: { uri, path, fields, schema } }})).toEqual({ ui: { currentForm: null } });
  });
