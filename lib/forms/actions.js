...
export function openForm(store, { uri, path, el, offset }) {
  ...
  store.commit(OPEN_FORM, { uri, path, fields: _.cloneDeep(group.fields), schema: group.schema, el, initialOffset: offset });
...
export function closeForm(store) {
  const currentForm = _.get(store, 'state.ui.currentForm'),
