...
describe('site', () => {
  describe('addPort', () => {
    ...
    it('adds port to uris', () => {
      expect(fn(`${domain}${path}`, { hostname: domain, port: '3000' })).to.equal(`${domain}:3000${path}`);
    });

    it('does not add port to uris on port 80', () => {
      expect(fn(`${domain}${path}`, { hostname: domain, port: '80' })).to.equal(`${domain}${path}`);
    });

    it('does not add port to uris that already have a port', () => {
      expect(fn(`${domain}:3000${path}`, { hostname: domain, port: '3000' })).to.equal(`${domain}:3000${path}`);
    });
  });
