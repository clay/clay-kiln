...
describe('history', () => {
  describe('updateStateWithEditAction', () => {
    const fn = lib.updateStateWithEditAction,
    ...
    test('will move the user to the end of the array and reset the updateTime if it already exists in the list', () => {
      const state = { users: [{ username: 'a@b', updateTime: 1234 }, { username: 'c@d' }] },
        result = fn(state, currentUser);

      expect(result.users[result.users.length - 1].username).toBe('a@b');
      expect(result.users[result.users.length - 1].updateTime).toBeGreaterThan(1234);
    });
    ...
    test('will set the updateTime timestamp', () => {
      ...
      expect(result.updateTime).toBeGreaterThanOrEqual(startTime);
    });
  });
