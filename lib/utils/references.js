...
export function getLayoutNameAndInstance(store) {
  const layoutURI = _.get(store, 'state.page.data.layout'),
    ...
    // todo: when we allow custom names, use that before falling back to the generated name

  return {
    name: layoutName,
    instance: layoutInstance,
    message: `You are currently editing the "${layoutInstance} (${layoutName})" layout. Changes you make will be reflected on all pages that use this layout.`
  };
}
