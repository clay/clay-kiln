...
describe('head components', () => {
  const headList = document.createDocumentFragment(),
    ...
    mid1 = document.createComment('data-uri="domain.com/components/sample-list-one"'),
  ...
  describe('getComponentListStart', () => {
    ...
    it('gets start of component list from path', () => {
      expect(fn('sample-component-list').nodeValue).to.equal('data-editable="sample-component-list"');
    });
  });
  ...
  describe('getComponentListStartFromComponent', () => {
    ...
    it('gets start of component list from node', () => {
      expect(fn(end)).to.equal('sample-component-list');
    });

    it('returns undefined if not in a list', () => {
      expect(fn(document.head)).to.equal(undefined);
    });
  });
  ...
  describe('getComponentListFragment', () => {
    ...
    it('gets document fragment from path', () => {
      expect(fn('sample-component-list').firstChild.nodeValue).to.equal('data-editable="sample-component-list"');
    });
  });
