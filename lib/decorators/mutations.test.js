...
define('preloader mutations', () => {
  it(`sets current selection for ${SELECT}`, () => {
    ...
    expect(lib[SELECT]({}, el)).to.eql({ ui: { currentSelection: el } });
  });

  it(`sets current selection for ${UN_SELECT}`, () => {
    expect(lib[UN_SELECT]({ ui: { currentSelection: document.createElement('div') }})).to.eql({ ui: { currentSelection: null } });
  });

  it(`sets current focus for ${FOCUS}`, () => {
    const focus = { uri: 'foo', path: 'bar' };

    expect(lib[FOCUS]({}, 'foo', 'bar')).to.eql({ ui: { currentFocus: focus }});
  });

  it(`sets current focus for ${UN_FOCUS}`, () => {
    const focus = { uri: 'foo', path: 'bar' };

    expect(lib[UN_FOCUS]({ ui: { currentFocus: focus }})).to.eql({ ui: { currentFocus: {} }});
  });
});
