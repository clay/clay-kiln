...
define('preloader mutations', () => {
  ...
  it(`sets current selection for ${SELECT}`, () => {
    expect(lib[SELECT]({}, { uri, el, parentField, parentURI })).to.eql({ ui: { currentSelection: { uri, el, parentField, parentURI } } });
  });

  it(`sets current selection for ${UN_SELECT}`, () => {
    expect(lib[UN_SELECT]({ ui: { currentSelection: { uri, el, parentField, parentURI } }})).to.eql({ ui: { currentSelection: null } });
  });
