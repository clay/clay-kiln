...
import {
  getPrevVisible, getNextVisible, getParentComponent, getFieldEl, getComponentEl
...
export function focus(store, {
  uri, path, initialFocus, el, offset, appendText, pos
}) {
    ...
    return store.dispatch('unfocus').then(() => {
      ...
      store.dispatch('setHash', { uri, path, initialFocus }).then(() => {
        store.dispatch('openForm', {
          uri, path, initialFocus, el: fieldEl, offset, appendText, pos
        }).then(() => {
