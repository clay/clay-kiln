...
import { findAll, create, insertAfter, replaceElement } from '@nymag/dom';
 ...
 * note: you can pass in an element to replace the current one with. this is how we re-render components
 * @param  {string} uri
 * @param {Element} [newEl] to replace the found elements with before decorating
 */
export function decorateURI(uri, newEl) {
...
/**
 * decorate a component instance, and its children
 * used when re-rendering a component
 * @param  {string} uri
 * @param  {Element} newEl
 */
export function decorateURIAndChildren(uri, newEl) {
  let children;

  // first, decorate the component
  decorateURI(uri, newEl);
  // then, find and decorate all of its children
  children = findAll(newEl, `[${refAttr}]`);
  if (children) {
    _.each(children, (child) => {
      decorate(child.getAttribute(refAttr), child);
    });
  }
}
