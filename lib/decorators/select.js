...
export function addSelector(uri, el) {
  let selector = new Selector({ store, componentEl: el }).$mount();
  ...
  if (parent) {
    ...
    if (!delegated.bodyMouseover) {
      delegated.bodyMouseover = document.body.addEventListener('mouseover', _.debounce((e) => {
        const closestComponent = closest(e.target, `[${refAttr}]`);

        if (closestComponent && isComponent(closestComponent.getAttribute(refAttr)) && getComponentName(closestComponent.getAttribute(refAttr)) !== 'clay-kiln' && !closestComponent.classList.contains('selected')) {
          store.dispatch('select', closestComponent);
        } else if (closestComponent && !isComponent(closestComponent.getAttribute(refAttr))) {
