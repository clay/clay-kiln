  ...
  if (parent) {
    ...
    if (!delegated.bodyMouseover) {
      delegated.bodyMouseover = document.body.addEventListener('mouseover', _.debounce((e) => {
        ...
        if (isComponent(closestComponent.getAttribute(refAttr)) && !closestComponent.classList.contains('selected')) {
          store.dispatch('select', closestComponent);
        } else if (!isComponent(closestComponent.getAttribute(refAttr))) {
