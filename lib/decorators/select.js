...
function isHoveringTowardsSelector(el, e, lastPos) {
  const pos = { x: e.clientX, y: e.clientY, el },
    lastSelector = lastPos.el && find(lastPos.el, '.quick-bar');

  if (lastPos.x === null) {
    // first time hovering, set the lastPos
    return true;
  }

  if (lastSelector) {
    // lastPos has an element with a selector. determine whether we're moving towards it
    const rect = lastSelector.getBoundingClientRect(),
      upperLeft = {
        x: rect.left,
        y: rect.top - 20
      },
      ...
      decreasingSlope = slope(pos, lowerLeft),
      increasingSlope = slope(pos, upperLeft),
      prevDecreasingSlope = slope(lastPos, lowerLeft),
      prevIncreasingSlope = slope(lastPos, upperLeft);

    console.log('\nSLOPE:')
    console.log(decreasingSlope, prevDecreasingSlope, decreasingSlope < prevDecreasingSlope)
    console.log(increasingSlope, prevIncreasingSlope, increasingSlope > prevIncreasingSlope)
    console.log(decreasingSlope < prevDecreasingSlope && increasingSlope > prevIncreasingSlope)
...
export function addSelector(uri, el) {
  ...
  if (!delegated.bodyMouseover) {
    let lastMousePos = { x: null, y: null, el: null };
    ...
    document.body.addEventListener('mouseover', _.debounce((e) => {
      ...
      if (closestComponent && isHoveringTowardsSelector(closestComponent, e, lastMousePos)) {
        lastMousePos = { x: e.clientX, y: e.clientY, el: closestComponent };
        ...
        // lastMousePos = { x: null, y: null, el: null };
