...
import { getComponentName, isComponent } from 'clayutils';
import store from '../core-data/store';
import { prependChild, closest } from '@nymag/dom';
import { refAttr, layoutAttr, editAttr, refProp, componentListProp, componentProp } from '../utils/references';
  ...
  const parentData = getData(parent),
    path = _.findKey(parentData, (prop) => {
      if (_.isArray(prop)) {
        ...
        const pageData = _.get(store, 'state.page.data');

        return pageData[prop] && !!_.includes(pageData[prop], child);
      }
    });
