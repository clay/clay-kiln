...
export function addSelector(uri, el) {
  const customButtons = _.get(window,'kiln.selectorButtons');
  let Selector, selector;

  // make sure the selector template registers custom selector buttons
  if (!_.isEmpty(customButtons)) {
    selectorTpl.components = _.merge({}, customButtons, selectorTpl.components);
  }

  Selector = Vue.component('selector', selectorTpl);
  selector = new Selector({ store, componentEl: el }).$mount();
