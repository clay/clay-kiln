...
function addDragula(el) {
  const drag = dragula([el], {
    ...
    invalid(selectedItem) {
      const isInsideField = selectedItem.classList.contains('field-main');

      if (isInsideField) {
        // set isDragging, so the click event that fires at the end of the drag
        // doesn't trigger unfocus. this allows users to select text in fields
        // by dragging their cursor outside the component (e.g. to the side)
        // without triggering dragdrop
        window.kiln.isDragging = true;
      }
  ...
  drag.on('drag', function (selectedItem, container) {
    ...
    // we triggered a successful drag (rather than selecting text),
    // so (if the current component's fields are focused)
    // the next time we click out it should unfocus the component
