...
import { editAttr, componentListProp, componentListClass, pageAreaClass, dropAreaClass, dragItemClass, dragItemUnsavedClass, refAttr, refProp } from '../utils/references';
...
function updateOrder(el) {
  const uri = getComponentEl(el).getAttribute(refAttr),
...
function addDragula(el) {
  const drag = dragula([el], {
    ...
    moves(selectedItem, container, handle) {
      ...
      return selectedItem.hasAttribute(refAttr) && isDraggable(handle, container);
    }
  });
  ...
  drag.on('drop', function (selectedItem, container) {
    // add the 'unsaved' class. this'll go away when the component re-renders
