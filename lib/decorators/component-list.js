 ...
 * @param  {Element} el
 */
function assertDirectParent(uri, path, el) {
  _.each(el.children, (child) => {
    if (child && !child.getAttribute(editAttr)) {
      throw new Error(`${getComponentName(uri)} → ${path} contains child elements that are not components! All child elements of component lists must be components.`);
    }
  });
}
...
export default function handler(uri, el) {
  ...
  if (schema && schema[componentListProp]) {
    assertDirectParent(uri, path, el);
