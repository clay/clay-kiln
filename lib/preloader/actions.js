...
import { addVariationField } from './get-variations';
...
export default function preload({ commit, dispatch }) {
  ...
  return props({
  ...
  }).then(({ pageState, pageData, allSites }) => {
    ...
    commit(PRELOAD_SUCCESS, _.assign(tabulaRasa, {
      ...
      schemas: _.mapValues(schemas, (schema, name) => {
        let processedSchema = addVariationField(schema, name, componentVariations);

        // console.group('setting schemas in state');
        // console.log("name:",name);
        // console.dir(schema);
        // console.groupEnd();

        // console.group('processedSchema:');
        // console.log(processedSchema);
        // console.groupdEnd();

        if (hasAnyBehaviors(processedSchema)) {
          return convertSchema(processedSchema, name);
        } else {
