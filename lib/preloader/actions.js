...
function getLayoutSchema(schemas, layoutRef) {
  const name = getLayoutName(layoutRef),
    schema = schemas[name];
...
export default function preload({ commit, dispatch }) {
  ...
  return props({
    ...
    layoutState: dispatch('fetchLayoutState', { uri: data._layout.uri, prefix: site.prefix, user: user }),
    ...
    commit(PRELOAD_SUCCESS, _.assign(tabulaRasa, {
      ...
      layout: _.assign({}, data._layout, { state: layoutState, data: composeLayoutData(layoutSchema, components, data), schema: layoutSchema }),
      user,
