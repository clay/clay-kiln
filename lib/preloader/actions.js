...
import { getComponentName, descProp, componentListProp, componentProp, refProp } from '../utils/references';
...
export default function preload({ dispatch, commit }) {
  ...
  return getPageState(data._self).then((pageState) => {
    ...
    // Open a form that was hashed in the url
    return dispatch('openHashedForm');
  })
}
