...
/**
 * get component kiln files so we can mount them on window.kiln.componentKilns
 * @return {object}
 */
function getComponentKilns() {
  window.kiln.componentKilns = window.kiln.componentKilns || {};
  if (_.isEmpty(_.get(window, 'kiln.componentKilns'))) {
    Object.keys(window.modules)
      .filter((key) => _.endsWith(key, '.kiln'))
      .forEach((key) => {
        window.kiln.componentKilns[key.replace('.kiln', '')] = window.require(key);
      });
  }

  return window.kiln.componentKilns;
}
...
function getSchemas(schemas, kilnjs) {
  return _.mapValues(schemas, (schema, name) => {
    ...
    if (kilnFile) {
      schema = kilnFile({..._.cloneDeep(schema), name });
    }
...
export default function preload({ commit, dispatch }) {
  ...
  return props({
    ...
    commit(PRELOAD_SUCCESS, _.assign(tabulaRasa, {
      ...
      user,
      schemas: getSchemas(schemas, kilnjs),
      locals,
      models,
      kilnjs,
