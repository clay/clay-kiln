...
function getComponentModels() {
  if (_.isEmpty(_.get(window, 'kiln.componentModels'))) {
    // window.modules is only available in the legacy Browserify pipeline.
    // In ESM mode (pack-next), window.kiln.componentModels is pre-populated
    // by the _kiln-edit-init bundle before DOMContentLoaded fires.
...
function getComponentKilnjs() {
  ...
  if (_.isEmpty(_.get(window, 'kiln.componentKilnjs'))) {
    // window.modules is only available in the legacy Browserify pipeline.
    // In ESM mode (pack-next), window.kiln.componentKilnjs is pre-populated
    // by the _kiln-edit-init bundle before DOMContentLoaded fires.
    Object.keys(window.modules || {})
